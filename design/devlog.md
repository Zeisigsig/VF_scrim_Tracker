...
### 후속: 디코닉 이름만 입력 허용 (커밋 `d7c32c8`)
디코닉이 `NN 이름`(생년 2자리 접두) 포맷이라, `97 승진` 대신 **`승진`만 쳐도** 매칭되게 `_resolve_nick` 개선: 완전일치 → 정규화일치 → **접두 제거 이름일치** 순. 접두 뗀 이름부는 현재 DB에서 중복 없음(77명). 나중에 동명이인이 생기면 **여러 명 매칭 시 후보를 리포트**(`ambiguous`)해 정확 입력을 유도(임의 선택 안 함). CLI·로컬 페이지 둘 다 표시.
...
### 핵심 버그: 최근 내전이 통째로 누락되던 문제 (2026-08-04, 커밋 `b51424a`)
사용자가 실전 실행 후 "왜 08-03 커스텀을 안 보고 옛날(4월) 것만 보냐"고 지적 → 실측으로 원인 규명.
- `_custom_items`가 쓰던 Henrik `get_matches(mode="custom")` 필터는 **`queue.id=='custom'` 인 것만** 반환한다. 그런데 **최근 내전은 `queue.id`가 `''`(빈값)** 이고(`queue.name=='Custom Game'`), 옛날 커스텀만 `id=='custom'`. → mode='custom' 필터가 최근 내전을 **통째로 누락**하고 옛날 것만 돌려줬음. (덤: 08-03 14:37/13:51은 `id=='competitive'`= 그냥 경쟁전, 내전 아님.)
- **수정**: `mode` 필터 제거하고 최근 전체를 받아 **`queue.name=='Custom Game'` 로 로컬 판별**(`_is_custom`). 경쟁전(`name=='Competitive'`) 자동 제외. 내전 직후 실행 전제라 최근 창에 그날 내전이 들어옴.
- `_MATCHES_PER_SEED` 2→6 (여러 맵 내전 커버). 소스가 최근 창뿐이라 옛날 커스텀은 못 딸려옴.
- **동일 함정이 `app/henrik/enrich.py`에도 남아있음**(mode='custom' 사용, 지문매칭용 별건 — 미수정, 나중에).

### UX: 디코닉 미매칭 비차단화 + 결과 판정 명확화 (2026-08-04, 커밋 `2cd69f4`·`b5751fd`)
사용자 불편: ① "디코닉 매칭 실패"가 뜨면 적재가 막힌 것처럼 보임 ② 경고들에 묻혀 **저장이 됐는지 안 됐는지**를 알 수 없음.
- 실측 확인: 미매칭 유저(콘도그/두국/kai)의 **발로닉 계정은 이미 존재**(pid 94/95/96) — 디코닉만 안 붙었을 뿐. confirm은 정상 동작하며 로스터 전원을 기존 플레이어에 연결(잡유저 생성 없음).
- 미매칭을 **경고→정보성**으로 문구 변경(CLI·webui). "발로닉으로 이미 등록된 경우 많음 · 적재하면 자동 연결" 명시.
- **결과 출력 개편**: 최상단에 **`✅ 저장됨 N개 · ❓ 확인 필요(아직 저장 안 됨) M개`** 판정. 적재와 무관한 정보(디코닉 미매칭·Riot 미연결·커스텀 없음·비표준 제외)는 맨 아래 **`참고 (조치 안 해도 됨)`** 섹션으로 분리. webui는 초록 배너 + 회색 참고카드.
- 멱등성 재확인: 이미 적재된(겹치는) 매치는 plan에서 `_exists`로 목록에 아예 안 뜨고, confirm으로 넣어도 `external_match_id` 유니크로 `[skip]` — 중복 저장 없음.
- 사용자 확인: "정말 내가 원하는 방식대로 작동" → 자동적재 기능 일단락. **다음 작업 예정: 로그인.**

## 2026-08-17 — 리네임(그리드→Tofu) 적재 누락 조사 + 진단 도구

### 발단
사용자가 자동적재(바탕화면 .bat) 후 병합하려 했는데, "그리드"가 인게임 닉을 **Tofu**로 바꾼 그 사람이 별도 유저로 **등록되지 않았다**고 함. 사용자 멘탈모델은 "확인(pending) 절차 = 참여자 검증 → 확인되면 **무조건 새 유저 등록**"인데 그 절차를 안 탄 것 같다는 의심.

### 코드로 확인한 실제 동작 (오해 교정)
- **로스터 매칭 기준은 닉/표시명이 아니라 Riot 계정** — `_resolve`(ingest_match.py:118)가 **① puuid → ② riot_name#tag** 순으로 기존 계정에 붙인다. 둘 다 미스일 때만 '미등록'.
- **`auto_register`(신규 유저 생성)는 '미등록(=기존 계정 매칭 실패)' 로스터에만** 일어난다. auto 경로(낯선 0명)·confirm 경로 **둘 다** 돈다. 즉 **"확인 절차 통과 = 무조건 신규 생성"이 아니다** — 이미 어떤 계정에 걸리는 사람은 확인을 거치든 안 거치든 그 유저로 흡수된다.

### 리네임 시 3가지 경우 (A/B/C)
- **A — 기존 유저로 흡수 (병합 불필요):** 바뀐 계정이 puuid(또는 name#tag)로 기존 계정(고가선)에 매칭 → 기록이 고가선으로 들어가고 Tofu 유저는 안 생김. 입력창에 그 사람 디코닉도 쳤다면 '낯선 0명'으로 잡혀 **확인 없이 자동적재**됐을 수 있음. 정상 동작.
- **B — 적재 자체가 안 됨:** 기존 계정에 puuid가 없고 name#tag도 안 맞으면(예: `고가선#3113` vs `Tofu#tag`) 그 사람이 '낯선'으로 분류 → 매치가 **pending("확인 필요")** 목록에만 뜸. 사용자가 그 매치를 **체크하고 "선택 적재"를 안 누르면** 적재 자체가 안 되어 Tofu도, 그 판 기록도 없음. — 사용자가 의심한 "절차를 안 탐"이 이 경우.
- **C — 별도 신규 등록:** 완전 미매칭이면 auto_register가 `Tofu` 유저를 새로 만듦 → 병합만 하면 됨.
- **적재된 매치라면 미등록 계정은 반드시 등록됨** — "적재는 됐는데 Tofu만 누락"은 로직상 안 나옴. Tofu가 없으면 그 매치가 A로 흡수됐거나 B로 미적재된 것.

### 발견한 취약점 (개선 여지)
- **puuid 백필 부재:** `_resolve`가 로스터를 name#tag로 기존 계정에 매칭해도 그 계정에 **puuid를 채워넣지 않는다**(ingest_match.py의 저장 경로에 갱신 없음). 그래서 puuid 없는 계정은 리네임 시 name#tag가 어긋나면 즉시 '낯선'이 됨 → 리네임 취약. (로컬 DB에서 `고가선#3113`의 puuid=None 확인.)

### 진단 도구
- **`app/tools/diag_rename.py` 추가** — VM 정본 DB를 **읽기 전용**으로 조회해 힌트 이름(그리드/고가선/Tofu)의 플레이어·Riot계정(puuid 유무)·경기수·최근매치·별도 Tofu 등록 여부를 출력하고 **A/B/C 판정 힌트**를 찍음. 쓰기 없음.
- 사용자가 GCP 브라우저 SSH로 접속하므로, 배포 없이 붙여넣을 수 있는 **자체완결 인라인 파이썬**(stdlib sqlite3, `file:...?mode=ro`)도 제공. VM 정본 경로 `/home/vf/VF_scrim_Tracker/data/scrim.db`, 파일이 vf 소유라 **`sudo -u vf`** 로 실행해야 함(본인 계정은 `/home/vf` 700 진입 불가 → "unable to open database file").
- **주의:** 로컬 `data/scrim.db`는 08-02 스냅샷이라 Tofu 데이터 없음 → 반드시 **VM 정본**에서 확인.

### 결론 (진단 쿼리 실행 결과) — 경우 B 확정
VM 정본 조회 결과: 고가선(id=19) `고가선#3113` **puuid 없음**, 마지막 매치 **08-10**, **Tofu 유저/계정 DB에 없음**. 문제의 내전은 **08-15**인데 그날 매치가 통째로 없음 → **pending("확인 필요")에 떴으나 "선택 적재"를 안 눌러 미적재**(경우 B). puuid가 없어 리네임된 name#tag(`Tofu#…`)가 기존 계정과 안 맞아 '낯선'으로 빠진 게 근인.
- **미해결 조치:** 08-15 매치 재적재 필요. 단 자동적재는 최근 창(~5~6판)만 봐서 지금은 밀려나 안 잡힐 수 있음 → 그 경우 `match_id`로 직접 적재. 사용자 수정 지시 대기.
- **재발 방지 후보:** name#tag로 기존 계정 매칭 시 **puuid 자동 백필**(다음 리네임에 대비) / 리네임 감지. 결과 확인 뒤 검토.

### 운영 메모
- 붙여넣기용 다중행 파이썬(heredoc)은 사용자 GCP 브라우저 SSH 화면에서 **indent가 한 단씩 밀려** 실행이 깨짐 → 앞으로 원격 스크립트는 **커밋→VM `git pull`→`uv run python -m ...`** 또는 들여쓰기 없는 한 줄 명령으로 제공.
- VM에서 vf 소유 파일 접근: `sudo -i -u vf`(비번 없이 vf 셸) 또는 명령 앞 `sudo -u vf`(소문자 u). 대문자 `-U`는 다른 옵션이라 usage 에러.

## 2026-09-18 — 개인 페이지 개편: 정렬/페이지네이션 · 최근 내전 · 맵별 킬/데스 히트맵

### 1. 상단바 아이디 → 내 페이지
`base.html` 의 `nav-user` 가 텍스트라 본인 프로필로 갈 길이 없었다 → `/player/{{ user.player_id }}` 링크(`.nav-me`)로.

### 2. 요원별 성과: 정렬 3종 + 5개씩 페이지
요원이 쌓이면서 표가 세로로 한없이 늘어지던 문제. **판수 / 승률 / 평균 점수** 탭 + `‹ 1/N ›`.
- 서버가 전체 행을 렌더하고 `player-panels.js` 는 **정렬·숨김만** 한다 → JS 실패해도 표는 보인다.
- 승률 동률은 **판수 많은 쪽 우선**(3판 100% 가 20판 70% 위로 오지 않게).
- 남의 페이지에선 평균 점수 열이 원래 가려지므로 그 정렬 탭도 숨긴다.

### 3. 최근 내전 2개 + 날짜별 내전 버튼
맵별/요원별 능력치 아래에 **최근 2경기 카드**(날짜·맵·승패·스코어·요원·K/D/A·ACS, 본인이면 표시점수) → 클릭 시 경기 상세.
그 아래 **날짜별 버튼**(`2026-07-22 어센트 승`) 최신순, 5개씩 `‹ ›`. 둘 다 기존 `history` 재사용 — **추가 쿼리 0**.

### 4. 맵별 킬·데스 히트맵 (신규)
"어느 **지점**에서 가장 많이 킬하고 죽었는지"를 미니맵에 시각화. 본인·어드민만(`can_see_scores`).

**데이터 — Henrik v4 상세에 좌표가 있다(실측 확인):**
- `kills[].location` = **희생자가 쓰러진 지점**(희생자는 `player_locations` 에 없음 → 확인됨)
- `kills[].player_locations[]` = 킬 순간 생존자 위치 → 여기서 **killer 좌표**를 집는다
- 2개월 전 경기도 상세 조회 OK, 킬 전원에 좌표 있음(7/09 스플릿 130/130).

**저장:** `kill_events` 테이블 + `matches.map_uuid`(마이그레이션 `0006_kill_events`).
`head_to_head.populate_match` 가 **이미 받아둔 상세**에 `store_kill_locations()` 를 끼워넣어 **추가 API 호출 0**. 경기 단위로 지우고 다시 넣어 멱등. 과거분은 기존 `backfill_h2h` 가 그대로 채운다(스킵 조건을 "h2h **와** kill_events 둘 다 있음"으로 바꿔 좌표 없는 경기는 자동 재방문). 로컬 48경기 백필 → **7352 이벤트**.
**원시 게임 좌표를 그대로 저장**한다 — 미니맵 변환은 표시 시점에 하므로 valorant-api 계수가 바뀌어도 재백필이 필요 없다.

**좌표 변환(실측 검증):**
```
px = y * xMultiplier + xScalarToAdd      ← 게임 y 가 이미지 x (축 스왑)
py = x * yMultiplier + yScalarToAdd
```
어센트 콜아웃 22개를 찍어 **22/22 정위치** 확인(스왑 없는 식은 7/22만 이미지 안). 실제 유저 킬/데스도 전부 통행 가능 영역에 떨어짐, 이미지 밖 0개.

**맵 메타:** `app/tools/sync_maps.py` → `app/web/static/maps.json`(맵 18개·콜아웃 346개, **한글** 콜아웃 — `?language=ko-KR`). 저장소에 커밋하므로 앱 기동 중 외부 호출 0. 미니맵 PNG 는 valorant-api CDN 핫링크.
**표시:** 서버는 맵별 원시 좌표만 넘기고 `player-heatmap.js` 가 변환·점 배치·최근접 콜아웃 집계("A 지점 4킬"). 점은 **% 좌표**라 이미지가 줄어도 따라간다(반응형 공짜).

### 곁가지로 발견한 것
- **`map_name` 오기 2건:** `matches.map_uuid`(Henrik 권위)와 대조하니 #12·#16(둘 다 07-10)의 **브리즈↔아이스박스가 서로 뒤바뀜**. 히트맵 탭 이름은 uuid 기준 공식명으로 덮어쓰게 했으나 **DB 값 자체는 미수정**(사용자 확인 대기, 정본은 VM).
- **삭제 경기 유령 행:** SQLite 는 `PRAGMA foreign_keys` 가 꺼져 있어 `ON DELETE CASCADE` 가 동작하지 않고 `HeadToHeadKill`·`KillEvent` 는 ORM 관계가 없다 → 경기 삭제 시 부가 행이 남아 라이벌 집계에 계속 반영되던 문제. `delete_match` 에서 두 테이블을 명시적으로 지우도록 수정.

### 배포 메모
VM 반영: `git pull` → **`uv run alembic upgrade head`**(0006) → `sudo systemctl restart vf` → **`uv run python -m app.henrik.backfill_h2h`**(정본 경기 수 × 약 2.2s, 좌표 없는 경기만 방문). 백필은 **VM 정본에서** 돌려야 반영된다(로컬은 스냅샷).

### 4-1. 방향 표시 추가 (같은 날 후속)
"데스는 어느 방향에서 맞았는지, 킬은 어딜 보고 쏜 건지"도 표시.

- **피격 방향(데스): 저장돼 있던 데이터로 즉시 가능.** `kill_events` 는 이벤트마다 **킬러 위치와 희생자 위치를 둘 다** 갖고 있다(7352건 중 7286건, 99.1%). 내 사망 지점 → 킬러 지점 벡터가 곧 "총알이 날아온 방향".
- **조준 방향(킬): `killer_view` 컬럼 추가**(마이그레이션 `0007_kill_view_angle`) + 재백필. Henrik `player_locations[].view_radians` 를 킬러 것만 저장한다.
- **불가: 죽는 순간 희생자 본인의 시선.** 희생자는 `player_locations` 에 아예 없다(한 경기 171킬 중 0건). Henrik 이 주지 않는 정보.

**조준각 좌표계 검증:** `view_radians` 는 게임 좌표계 기준이라 미니맵 축으로 옮길 때 또 뒤집힐 수 있다. "킬 순간 조준은 희생자 쪽을 향한다"를 기준으로 후보 4개를 7286건에 대조:

| 후보 | 각도차 중앙값 | 30° 이내 |
|---|---|---|
| **(sin v·xm, cos v·ym)** | **0.3°** | **99.9%** |
| (cos v·xm, sin v·ym) | 89.5° | 13.0% |
| (-cos v·xm, -sin v·ym) | 90.5° | 12.5% |
| (-sin v·xm, -cos v·ym) | 179.7° | 0.0% |

→ 위치 변환과 같은 축 스왑. 맵마다 `xm` 부호가 다르므로(`ym` 은 항상 음수) 계수를 그대로 태워야 한다.

**표시:** 점에서 뻗는 짧은 그라데이션 선(`.heat-ray`, CSS `rotate`). 미니맵이 전 맵 1024×1024 정사각이라 % 단위 delta 로 화면 각도를 그대로 낼 수 있다. `방향 표시` 체크박스로 토글, 점 hover 시 `← B 메인 쪽에서 피격` / `→ 상대 A 지점`.
**페이로드:** `kills=[내x, 내y, 조준각, 상대x, 상대y]`, `deaths=[내x, 내y, 킬러x, 킬러y]` (맵 하나 12.7KB 수준).

**곁가지:** `sync_maps` 가 **좌표 계수 0인 맵**(데스매치 전용 — 디스트릭트·카즈바·드리프트·글리치·피아자)을 걸러내도록 수정. 계수가 0이라 히트맵을 그릴 수 없다(맵 18 → 13개).

### 4-2. 마무리 수정 (사용자 확인 후)
- **날짜별 내전 페이지네이션이 먹지 않던 버그.** 버튼이 5개씩 안 끊기고 전부 보였다. 원인은 JS 가 아니라 **CSS 우선순위** — `hidden` 속성의 `display:none` 은 브라우저 기본 스타일이라 작성자 규칙 `.match-btn{display:inline-flex}` 에 밀린다. 요원별 표가 멀쩡했던 건 `<tr>` 에 display 지정이 없어서. 전역 `[hidden]{display:none!important}` 로 해결(`.pager` 가 5개 이하일 때 안 숨던 것도 같이 고쳐짐).
- **날짜별 내전 4개씩으로.** 내전은 짝수 단위로 열리므로 5 → 4. 요원별 성과는 5 유지(경기 수 얘기가 아니라서).
- **긴 이름이 줄바꿈되던 문제.** `.match-btns` 를 `flex-wrap` → **4칸 고정 그리드**로. 이름 길이와 무관하게 한 줄 4개, 넘치면 맵 이름만 말줄임(`text-overflow`), 승패는 오른쪽 끝 고정. 모바일 2칸 / 400px 이하 1칸.
- 작업 중 400px 미디어쿼리를 중간에 삽입해 히트맵 모바일 규칙 2개가 그 블록으로 딸려 들어갔던 것 되돌림.

---

## 2026-09-19 — 인게임 닉 변경 추종 (Monalisa → Orbit)

### 증상
어떤 유저가 인게임 닉을 `Monalisa` → `Orbit` 으로 바꿨는데, **`Orbit` 으로 검색하면 그 유저가 나오지만 표시되는 이름은 계속 `Monalisa`** 였다.

### 원인 — 검색과 표시가 서로 다른 필드를 본다
- 표시는 `Player.label` 하나만 쓴다(`디코닉 (발로닉)` / 발로닉).
- 검색 건초더미는 `display_name + discord_name + aliases`(`players.js`).
- 새 닉 `Orbit` 은 **별칭으로만** 들어와 있었다. 검토창 확정 시 `register_alias()` 가 저장하기 때문(enrich 가 OCR 닉을 Henrik 의 현재 닉으로 이미 교정해서 넣는다). 별칭은 검색 대상이지만 표시에는 안 쓰인다.
- 적재 자체는 멀쩡했다. `_resolve()` 가 **puuid 우선**이라 닉이 바뀌어도 같은 사람으로 붙는다. 다만 **`display_name` 을 갱신하는 코드가 어디에도 없었다** — 쓰기는 어드민 수정 폼(`routes.py`) 한 곳뿐.

### 더 큰 문제 — puuid 없는 유저는 리네임하면 갈라진다
`_resolve()` 는 puuid 가 없으면 `(riot_name, riot_tag)` 로 떨어진다. puuid 가 비어 있는 사람이 닉을 바꾸면 이 키가 깨져 미해결 → `_auto_register()` 가 **신규 유저를 만든다.** 로컬 스냅샷 기준 89명 중 puuid 보유가 46명뿐이었다. 2026-08-17 "그리드→Tofu 적재 누락"으로 의심했던 건도 누락이 아니라 이것이었을 가능성이 크다.

→ **puuid 전수 보강이 리네임 추종의 선행 조건.**

### 1. `app/tools/backfill_puuid.py` (신규)
`puuid` 가 빈 Riot 계정을 `name#tag` 로 조회해 채운다. puuid 는 계정 불변 ID라 **한 번 채우면 이후 닉을 몇 번 바꾸든 영구히 따라간다.**
- 조회 성공 = 저장된 `name#tag` 가 현재값이라는 뜻 → 그 김에 `sync_player_nick()` 으로 발로닉도 맞춘다.
- **이미 닉을 바꾼 사람은 저장된 `name#tag` 가 낡아 404 로 실패한다**(조회 방향이 `name#tag→puuid` 뿐이라 구조적 한계). 실패 목록을 따로 출력한다. 그냥 두어도 그 사람이 낀 내전이 한 번 적재되면 로스터에서 puuid 가 채워진다.
- 같은 puuid 를 다른 계정 행이 이미 쥐고 있으면 덮어쓰지 않고 **중복 유저(병합 대상)로 보고**한다.

### 2. `services.sync_player_nick()` (신규)
Henrik 로스터의 현재 Riot 닉으로 발로닉·Riot 계정을 따라가게 한다.
- 옛 닉·새 닉 **둘 다** 별칭으로 남긴다. 옛 닉은 과거 스크린샷 OCR 이 계속 붙도록, 새 닉은 **`match_nickname()` 이 별칭 정확일치만 자동매칭**하므로 다음 스크린샷이 자동으로 붙도록.
- Riot 계정 행은 **제자리 수정**. `(player_id, riot_name, riot_tag)` 유니크라 새 행을 넣으면 같은 사람 계정이 둘로 늘어난다.
- **별칭 충돌**(그 닉이 이미 남의 것)은 전역 유니크라 조용히 넘어가면 OCR 오매칭이 남는다 → 뺏지 않고 경고를 뱉는다.
- **부계정(계정 2개 이상)은 발로닉 자동 갱신 보류.** 어느 계정 닉을 대표로 쓸지 규칙이 없어, 부계로 낀 판이 부계 닉으로 덮어쓰는 것을 막는다. 알림만.
- **puuid 로 신원이 확정된 Henrik 로스터에서만 호출한다.** OCR 닉으로 부르면 깨진 닉이 발로닉을 덮어쓰므로, 웹 확정 경로(`routes.py`)는 지금대로 별칭만 등록한다.

### 3. 적재 훅 (`ingest_match._sync_nicks`)
`_ingest_confirmed`·`_ingest_review` 양쪽에서 로스터 해석 직후 호출(= CLI·`auto_ingest` 모두 커버). 검토 모드는 `names` 계산 **전에** 불러야 검토창이 새 닉으로 자동매칭된다. 변경은 조용히 넘기지 않고 `[닉]` 으로 출력한다 — 어드민이 손으로 맞춰둔 이름이 되돌아갈 수 있어 흔적이 남아야 한다.

### 감수하기로 한 것
- 과거 경기도 **현재 닉**으로 보인다. `match_players` 에 당시 닉이 없어 복원 불가. 대부분 최신 닉이 낫다고 판단.
- 어드민이 손으로 고친 발로닉은 다음 적재가 Riot 닉으로 되돌린다("수동 지정" 플래그가 없다). 로그로 흔적은 남는다.

### 곁가지
`diag_rename.py` 가 `Player.riot_name` 을 읽어 **`AttributeError` 로 즉사**하던 버그 수정(Riot 닉은 `PlayerRiotAccount` 에 있다). 08-17 에 만들고 VM 에서 실행해 본 적이 없었던 것.

### 4. 갈라진 중복 유저 병합 (같은 날 이어서)

리네임 추종을 붙여도 **이미 갈라진** 중복은 남는다. 그걸 찾아 합치는 흐름.

#### 4-1. `merge_players` 구멍 3개 (자동화 이전에 먼저)
인메모리 DB로 `Tofu`(새) → `그리드`(옛) 병합을 실측하니 이렇게 나왔다:
```
병합 후 Riot 계정: [(1, '그리드', None)]      ← 새 유저의 puuid 가 통째로 소멸
병합 후 User.player_id: [('토푸', 2)]        ← 2번은 방금 삭제된 플레이어
```
- **Riot 계정 소멸.** `Player.riot_accounts` 에 `cascade="all, delete-orphan"` 이 걸려 있어 source 삭제 시 계정도 지워진다. 갈라진 중복은 **새 쪽이 puuid 를 쥐고 있어서**, 그대로 병합하면 리네임 추종의 근거를 잃고 다음에 또 갈라진다. → 먼저 target 으로 옮기고, 같은 계정이 양쪽에 있으면 puuid 보유 쪽을 남긴다.
- **로그인 계정 미이전.** `User.player_id` 가 죽은 id 를 가리켜 로그인이 깨지고, `unique` 제약 때문에 재발급도 막힌다. → 옮긴다. 양쪽 다 계정이 있으면 어느 쪽을 버릴지 기계가 못 정하므로 `ValueError` 로 거부.
- **`head_to_head_kills`·`kill_events` 미이전.** 경기 삭제에는 있는데 병합에는 빠져 있어 라이벌 집계·히트맵에 죽은 id 가 남았다. → 이전. h2h 는 `(match, killer, victim)` 유니크라 충돌 시 `kills` 를 **합산**하고, 리맵 결과 `killer==victim` 이 되는 비정상 행은 버린다.

이건 자동화와 무관하게 **어드민 병합 버튼에 원래 있던 버그**다.

#### 4-2. `app/tools/merge_renamed.py` — 동일인 판정
진짜 어려운 건 머지 규칙이 아니라 **"이 둘이 같은 사람"을 아는 것**이다. puuid 가 같으면 확정인데, 갈라진 케이스는 정확히 옛 유저에 puuid 가 없어서 갈라진 거라 비교가 성립하지 않는다(옛 `name#tag` 는 죽어서 404).

**풀이:** 옛 유저가 뛴 확정 경기를 Henrik 상세로 다시 받아 **저장된 스탯(K/D/A/ACS) 지문**으로 로스터 항목을 특정하면 그 항목의 puuid 가 나온다. `|ΔK|+|ΔD|+|ΔA|+0.5·|ΔACS| ≤ 6.0` 이고 2등과 4.0 이상 벌어질 때만 채택(동점이면 다음 경기로).

**함정:** 처음엔 `Match.external_match_id` 만 봤는데 로컬 58판 중 **2판**뿐이었다. 스크린샷 업로드분은 enrich 가 지문매칭으로 찾은 ID를 `extraction_raw.henrik_match_id` 에 넣어 둔다(48판). 양쪽을 다 보게 고치니 표본 4명이 전부 복원됐다.

**생존자 규칙:** `로그인 계정 보유 > 수동 티어 보유 > 판수 많음 > 먼저 생성됨`. `created_at` 만으로 고르지 않는 이유는, 보통 옛 유저가 맞지만 수동 티어·로그인이 새 쪽에 붙어 있으면 그것을 잃는 손실이 더 크기 때문.

**병합은 CLI 가 하지 않는다.** 되돌릴 수 없는 작업이 일상 적재에 묻히면 사고 추적이 안 되므로, 후보만 `app_settings` KV(`merge_candidates`)에 JSON 으로 저장한다. 후보는 많아야 몇 건이라 전용 테이블을 만들지 않았다.

#### 4-3. 유저 관리 UI
- **puuid 읽기 전용 표시.** Riot 칩에 앞 8자 + `title` 에 전체. 손으로 넣을 값이 아니라 입력 요소를 두지 않았다. 없으면 `puuid 없음` 경고색 — **리네임 내성이 있는지를 한눈에** 보는 게 목적이다.
- **'병합 후보' 패널**(페이지 맨 아래, 어드민 전용). 흡수될 쪽/남길 쪽·판수·근거·Riot ID 를 보여주고 [병합]/[기각]. 병합은 `merge_players` 후 **`sync_player_nick` 으로 현재 닉을 씌운다** — `merge_players` 는 생존자 이름을 그대로 두기 때문에, 이게 없으면 표시명이 옛 닉으로 남아 원래 고치려던 문제가 그대로 재현된다. 이미 병합·삭제된 후보는 렌더 시 걸러져 목록이 저절로 정리된다.

### 5. 히트맵 확대 보기

미니맵이 작아 점이 밀집한 구역을 읽기 어렵다는 피드백.

**복제하지 않고 옮긴다.** 오버레이용으로 `.heat-map` 을 복제하면 모드(킬만/데스만)·방향 토글 상태를 양쪽에 동기화해야 한다. 대신 **원본 노드를 오버레이 슬롯으로 `appendChild` 했다가 닫을 때 제자리로 되돌린다** — `render()` 가 들고 있는 `img`/`dots` 참조가 그대로라 재렌더도 동기화도 없다. 되돌리기는 `{node, parent, nextSibling}` 을 기록해 **옮긴 역순**으로 `insertBefore`(서로의 nextSibling 이 제자리에 있어야 하므로).

모드 탭 묶음과 방향 체크박스도 같이 옮겨 확대 상태에서 조작할 수 있게 했다. 맵 선택 탭은 두고 왔다(닫고 고르면 된다).

**크기가 저절로 맞는다.** 점은 `%` 좌표라 래퍼가 커지면 그대로 따라오고, 점(9px)·방향선(20px)은 px 고정이라 **상대적으로 작아진다** → 밀집 구역의 겹침이 풀린다. 슬롯은 `min(92vw, calc(100vh - 130px))` 정사각 — 미니맵이 전부 1024² 정사각이라 왜곡이 없다.

닫기는 배경 클릭·× 버튼·ESC. 확대 중 지도 클릭은 닫지 않는다(점 툴팁을 확인해야 하므로) — 배경(`e.target === zoom`)일 때만 닫는다.

`player-heatmap.js?v=2` → `?v=3` (캐시 무효화 — 이전에 CSS/JS 캐시로 한 번 헤맸다).
